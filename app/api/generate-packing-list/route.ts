...
### 5. 物品描述必须具体可执行
- **禁止**模糊表述：如「换洗衣物」「常用药品」「洗漱用品」
- **必须**写清品名 + 数量/规格 + 括号内注明用途或原因，例如：
  - ✅「轻便速干 T 恤 3 件」「内裤 5 条」「袜子 5 双」
  - ✅「折叠晴雨伞 1 把（梅雨季/多雨地区必备）」
  - ✅「SPF50+ 防晒霜 1 支（当地紫外线强）」
  - ✅「欧标转换插头 1 个（欧洲电压 220V，插座形状不同）」

### 6. 不列显而易见的随身物品
- **禁止**列入用户日常随身携带、无需特别提醒的物品，包括：手机、钱包、日常眼镜、现金（除非是该目的地特别需要准备外币现金）
- 充电器**必须列入**，但要写具体：如「手机充电器 1 个」「多口 USB 充电头 1 个」

### 7. 不列酒店已提供的洗漱用品
- 国内目的地（中国大陆境内）短途旅行（7天以内），**不列**牙刷、牙膏、沐浴露、洗发水、浴巾等酒店标配物品
- 国际目的地或民宿/青旅出行，可根据实际情况列入

### 8. 行程准备类写具体行动，不只列 App 名称
- **禁止**只写 App 名称，如「下载携程」「装支付宝」
- **必须**写清楚要做什么事，例如：
  - ✅「提前在 Visit Japan Web 完成入境信息申报（日本入境必须）」
  - ✅「开通国际漫游或购买当地 eSIM（落地即可用网）」
  - ✅「下载离线地图（Google Maps 或 Maps.me，防止无网络）」
  - ✅「铁路 12306 提前购买高铁票」
